...
def initialize_translation():
    ...
    try:
        ...
        # Step 4: Create translation engines
        logger.info("Loading translation engines...")
        TRANSLATOR_FR_EN = argostranslate.translate.get_translation('fr', 'en')
        TRANSLATOR_EN_FR = argostranslate.translate.get_translation('en', 'fr')

        if TRANSLATOR_FR_EN is None:
            logger.error("Failed to load French to English translator")
        if TRANSLATOR_EN_FR is None:
            logger.error("Failed to load English to French translator")
